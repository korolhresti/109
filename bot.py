...
# Removed 'create_user' from the import list as it's not defined in database.py
...
# Нова команда для ручного запуску парсингу новин (тільки для адмінів)
@router.message(Command("run_parse_job"), is_admin_check)
async def run_parse_job_command(message: Message):
    await message.answer("Запускаю завдання парсингу новин вручну. Це може зайняти деякий час...")
    try:
        await parse_all_sources_job()
        await message.answer("Завдання парсингу новин завершено.")
    except Exception as e:
        logger.error(f"Manual parse job failed: {e}", exc_info=True)
        await message.answer(f"Помилка під час ручного запуску парсингу: {e}")

# Нова команда для ручного запуску публікації новин (тільки для адмінів)
@router.message(Command("run_publish_job"), is_admin_check)
async def run_publish_job_command(message: Message):
    await message.answer("Запускаю завдання публікації новин вручну. Перевірте канал.")
    try:
        await publish_news_to_channel_job()
        await message.answer("Завдання публікації новин завершено.")
    except Exception as e:
        logger.error(f"Manual publish job failed: {e}", exc_info=True)
        await message.answer(f"Помилка під час ручного запуску публікації: {e}")
...
async def process_telegram_source_url(message: Message, state: FSMContext):
    url = message.text.strip() # Видаляємо пробіли
    if not url:
        await message.answer("URL не може бути порожнім. Будь ласка, введіть дійсний URL.",
                             reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                 [InlineKeyboardButton(text="⬅️ Скасувати", callback_data="settings")]
                             ]))
        return
...
async def process_telegram_source_name(message: Message, state: FSMContext):
    name = message.text.strip() # Видаляємо пробіли
    if not name:
...
async def process_telegram_source_category(message: Message, state: FSMContext):
    ...
    if not category:
        await message.answer("Категорія джерела не може бути порожньою. Будь ласка, введіть категорію.",
                             reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                 [InlineKeyboardButton(text="⬅️ Скасувати", callback_data="settings")]
                             ]))
        return
...
async def process_telegram_source_parse_interval(message: Message, state: FSMContext):
    try:
        parse_interval = int(message.text.strip())
        if parse_interval <= 0:
            raise ValueError
    except ValueError:
    ...
    try:
        ...
        logger.error(f"Error adding source via Telegram: {e}", exc_info=True)
        await message.answer(f"Помилка при додаванні джерела: {e}. Спробуйте ще раз.",
                             reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                 [InlineKeyboardButton(text="⬅️ Скасувати", callback_data="settings")]
                             ]))
    finally:
...
async def admin_source_stats_callback_handler(callback: CallbackQuery):
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=kb)
    await callback.message.edit_text(stats_text, reply_markup=markup, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
...
async def process_source_parse_interval(message: Message, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Error adding source via Telegram (admin): {e}", exc_info=True)
        await message.answer(f"Помилка при додаванні джерела: {e}. Спробуйте ще раз.",
                             reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                 [InlineKeyboardButton(text="⬅️ Скасувати", callback_data="admin_panel")]
                             ]))
...
async def process_new_source_value(message: Message, state: FSMContext):
    ...
    new_value = message.text.strip() # Видаляємо пробіли

    if not new_value and field in ['url', 'name', 'category', 'language', 'status']:
        await message.answer(f"Поле '{field}' не може бути порожнім. Будь ласка, введіть значення.",
                             reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                 [InlineKeyboardButton(text="⬅️ Скасувати", callback_data="admin_panel")]
                             ]))
        return
    ...
    try:
        ...
        logger.error(f"Error updating source via Telegram (admin): {e}", exc_info=True)
        await message.answer(f"Помилка при оновленні джерела: {e}. Спробуйте ще раз.",
                             reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                                 [InlineKeyboardButton(text="⬅️ Скасувати", callback_data="admin_panel")]
                             ]))
    finally:
...
async def startup_event():
    ...
    try:
        await get_db_pool()
        logger.info("Database pool initialized during startup.")
    except Exception as e:
        logger.critical(f"Failed to initialize database pool during startup: {e}", exc_info=True)
    ...
    if not TELEGRAM_BOT_TOKEN: # Check if token is empty or None
        ...
        if not WEB_APP_URL:
            ...
            try:
                ...
                await bot.set_webhook(webhook_url) # UNCOMMENTED
...
async def shutdown_event():
    ...
    try:
        ...
        if TELEGRAM_BOT_TOKEN: # Check if token is not empty or None
            ...
            await bot.delete_webhook() # UNCOMMENTED
            logger.info("Telegram webhook successfully deleted.")
    except Exception as e:
